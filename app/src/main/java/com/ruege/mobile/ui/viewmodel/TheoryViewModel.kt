...
class TheoryViewModel @Inject constructor(
    ...
    private fun loadTheoryTopics() {
        viewModelScope.launch {
            ...
            theoryRepository.getTheoryTopicsStream().collect { entities ->
    ...
    fun loadTheoryContent(contentId: String) {
        viewModelScope.launch {
            ...
            try {
                val downloadedTheory = theoryRepository.getDownloadedTheoryContent(contentId)
                ...
                val theoryDto = theoryRepository.getTheoryContentById(contentId)
                _isLoading.value = false
    ...
    fun downloadTheory(contentId: String) {
        viewModelScope.launch {
            theoryRepository.downloadTheory(contentId).collect { result ->
                when (result) {
    ...
    fun deleteDownloadedTheory(contentId: String) {
        viewModelScope.launch {
            theoryRepository.deleteDownloadedTheory(contentId).collect { result ->
                when (result) {
    ...
    fun getDownloadedTheory(contentId: String): LiveData<DownloadedTheoryEntity> {
        return theoryRepository.getDownloadedTheory(contentId)
    }

    fun refreshTheoryTopics() {
        viewModelScope.launch {
            try {
                theoryRepository.refreshTheoryTopics()
            } catch (e: Exception) {
    ...
    fun downloadSelectedTheories() {
        viewModelScope.launch {
            ...
            for (item in selectedItems) {
                theoryRepository.downloadTheory(item.contentId)
    ...
    fun clearAllDownloadedTheory() {
        viewModelScope.launch {
            try {
                theoryRepository.clearAllDownloadedTheory()
            } catch (e: Exception) {
                Timber.e(e, "Failed to clear all downloaded theory")
                _errorMessage.value = "Ошибка при очистке теории"
            }
        }
    }
} 