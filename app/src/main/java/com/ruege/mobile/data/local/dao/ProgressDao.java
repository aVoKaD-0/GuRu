...
public interface ProgressDao {
    ...
    @Query("SELECT content_id FROM progress")
    ...
    @Query("SELECT AVG(percentage) FROM progress")
    LiveData<Integer> getAverageProgress();

    @Query("SELECT COUNT(*) FROM progress WHERE completed = 1")
    LiveData<Integer> getCompletedContentCount();
    
    @Query("SELECT COUNT(*) FROM progress")
    LiveData<Integer> getTotalContentCount();
    

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void insertOrUpdateAll(List<ProgressEntity> progressList);
} 