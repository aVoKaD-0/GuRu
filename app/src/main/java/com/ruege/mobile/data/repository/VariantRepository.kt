                ...
                if (response.isSuccessful) {
                    ...
                    if (dtoList.isNotEmpty()) {
                        ...
                        if (variantsToUpdate.isNotEmpty()) {
                            ...
                            Log.d(TAG, "Updated ${variantsToUpdate.size} variants.")
                        }
                        
                        if (variantsToInsert.isNotEmpty()) {
                            variantDao.insertOrUpdateVariants(variantsToInsert)
                            Log.d(TAG, "Inserted ${variantsToInsert.size} new variants.")
                        } else {
                            Log.d(TAG, "не знаю что вывести")
                        }

                    } else {
                        Log.d(TAG, "Fetched and saved/updated 0 variants from DB. Error/")
                    }
                } else {
    ...
    suspend fun deleteDownloadedVariant(variantId: Int): Resource<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                Log.d(TAG, "Successfully deleted downloaded data for variant $variantId")
                Resource.Success(Unit)
            } catch (e: Exception) {
                Log.e(TAG, "Error deleting downloaded variant data for variant $variantId", e)
