...
interface VariantTaskDao {
    ...
    @Query("SELECT * FROM variant_tasks WHERE variant_task_id = :taskId")
    suspend fun getTaskById(taskId: String): VariantTaskEntity?
    ...
    @Query("UPDATE variant_tasks SET check_id = :checkId WHERE variant_task_id = :variantTaskId")
    suspend fun updateCheckId(variantTaskId: Int, checkId: String?)
}