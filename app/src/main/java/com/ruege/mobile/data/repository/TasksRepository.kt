...
class TasksRepository @Inject constructor(
    ...
    fun getTaskDetail(taskIdString: String): Flow<Result<TaskItem>> = flow {
        ...
        if (localTask != null) {
            ...
            val textDto = localText?.let { TextDataDto(it.textId.toInt(), it.content, null, null, null) }
    ...
    private fun TextDataDto.toEntity(): TaskTextEntity {
    ...
    private fun TaskEntity.toTaskItemWithText(textDto: TextDataDto?): TaskItem {
    ...
    suspend fun clearAllDownloadedTasks() {
        withContext(Dispatchers.IO) {
            taskDao.deleteAll()
            taskTextDao.deleteAll()
            contentDao.updateAllDownloadStatusByType("task_group", false)
        }
    }
}